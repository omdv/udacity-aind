...
class SelectorBIC(ModelSelector):
    ...
    def select(self):
        ...
        # initiate result
        max_BIC = np.float("+inf")
        best_model = None

        # cycle over a number of components
        for n_components in range(self.min_n_components, self.max_n_components+1):
            try:
                _m = GaussianHMM(n_components=n_components, covariance_type="diag", n_iter=1000,
                                        random_state=self.random_state, verbose=False).fit(self.X, self.lengths)
                log_loss = _m.score(self.X, self.lengths)
                BIC = -2 * log_loss + n_components * np.log(len(self.X))
            except:
                BIC = np.float("+inf")
        ...
        for n_components in range(self.min_n_components, self.max_n_components+1):
            try:
                _m = GaussianHMM(n_components=n_components, covariance_type="diag", n_iter=1000,
                                        random_state=self.random_state, verbose=False).fit(self.X, self.lengths)
                log_loss = _m.score(self.X, self.lengths)
                DIC = -2 * log_loss + n_components * np.log(len(self.X))
            except:
                DIC = np.float("+inf")
...
class SelectorCV(ModelSelector):
    ...
    def select(self):
        ...
        #hmmlearn complains about division, but still performs training
        warnings.filterwarnings("ignore", category=RuntimeWarning)
        ...
        for n_components in range(self.min_n_components, self.max_n_components+1):
            ...
            for cv_train_idx, cv_test_idx in split_method.split(self.sequences):
                ...
                if len(X_train) > n_components:
                    ...
                    log_loss = hmm_model.score(X_test, L_test)
                else:
                    log_loss = np.float("+inf")
