...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # iterate over all possible actions and create an action node
        for _a in self.all_actions:
            a_node = PgNode_a(_a)
            # test if subset
            if a_node.prenodes.issubset(self.s_levels[level]):
                # connect and append
                for _s in self.s_levels[level]:
                    _s.children.add(a_node)
                    a_node.parents.add(_s)
    ...
    def add_literal_level(self, level):
        ...
        # initiate
        s_nodes = set()
        # loop through actions from previous level
        for _a in self.a_levels[level - 1]:
            for _s in _a.effnodes:
                # add and connect levels
                _a.children.add(_s)
                _s.parents.add(_a)
                s_nodes.add(_s)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for _e in a1.effect_add:
            if _e in a2.effect_rem:
                return True

        for _e in a2.effect_add:
            if _e in a1.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for _e in a1.effect_add:
            if (_e in a2.precond_pos) or (_e in a2.precond_neg):
                return True

        for _e in a2.effect_add:
            if (_e in a1.precond_pos) or (_e in a1.precond_neg):
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # loop through parent nodes and check if mutex        
        for _a1 in node_a1.parents:
            for _a2 in node_a2.parents:
                if _a1.is_mutex(_a2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for _s1 in node_s1.parents:
            for _s2 in node_s2.parents:
                if not _s1.is_mutex(_s2):
                    return False 
    ...
    def h_levelsum(self) -> int:
        ...
        # follow AIMA pg 382, iterate over levels, check if goal state appears
        # and if the problem is solvable

        has_goal = False
        for _g in self.problem.goal:
            goal = PgNode_s(_g, True)
